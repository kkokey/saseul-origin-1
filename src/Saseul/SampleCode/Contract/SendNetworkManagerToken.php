...
class C3 extends Contract
{
    public const TYPE = 'SendNetworkManagerToken';
    ...
    {
        S1::GetInstance()->loadToken($this->from, $this->network_manager_token_name);
        S1::GetInstance()->loadToken($this->to, $this->network_manager_token_name);
    }

    public function _makeDecision(): string
    {
        $from_balance = S1::GetInstance()->getBalance($this->from, $this->network_manager_token_name);
    ...
    public function _setStatus(): void
    {
        ...
        S1::GetInstance()->setBalance($this->from, $this->network_manager_token_name, $from_balance);

        # add & save
        $to_balance = S1::GetInstance()->getBalance($this->to, $this->network_manager_token_name);
        $to_balance = bcadd($to_balance, $this->amount);
        S1::GetInstance()->setBalance($this->to, $this->network_manager_token_name, $to_balance);

        # from role
        $from_balance = S1::GetInstance()->getBalance($this->from, $this->network_manager_token_name);

        if (bccomp($from_balance, '0') === 0) {
            Authority::GetInstance()->removeAuthority($this->from, Title::NETWORK_MANAGER);
        }
        ...
        if (bccomp($to_balance, '0') === 1) {
            Authority::GetInstance()->setAuthority($this->to, Title::NETWORK_MANAGER);
        }
    }
}
