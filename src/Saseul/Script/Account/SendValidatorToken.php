...
class SendValidatorToken extends Script
{
    function main()
    {
        $address = $this->ask('Please type address to send vt. ');
        $amount = $this->ask('Please type amount to send vt. ');
    ...
    {
        $type = 'SendValidatorToken';
        $transaction = [
            'type' => $type,
            'version' => Version::CURRENT,
            'from' => Env::getAddress(),
            'to' => $address,
            'timestamp' => DateTime::microtime() + (2 * Rule::MICROINTERVAL_OF_CHUNK),
